...
void xMaterial2::Density(int ud)
{
   if (ud)
      Ro = N * A / (U_NA * gr);
   else
...
void xMaterial2::RMSANGLE(U_Energy &e, doubleU &L2)
{
   doubleU db(U1_);
   doubleU Uin(U1_);
   doubleU a1(cm_);
   ...
   Omega0 = BC * L2 * e.Z * e.Z / (e.Beta * e.Beta);

   ChiCC = 0.39612 * U_Pow((ZSP * Ro(g_ / (cm_ ^ 3))), 0.5);
   ChiC = ChiCC * e.Z * U_Pow(L2(cm_), 0.5) / (e.A * 931.49432 * e.Gamma * e.Beta);
   ...
   Teta2 = 2 * U_pi * N * L2 * ((Z * e.Z * U_rp / e.A / (e.Beta2 * e.Gamma)) ^ 2) *
           (U_Ln(Ksi) - 1 + db);
}

void xMaterial2::Bethe(U_Energy &e, doubleU &L2)
{
   doubleU Ra;
   ...
   */
   ...
   GGG = Emax / (Emax + I);
   Psi = 0.1535 * Ra * e.Z * e.Z * Z * Ro * L2 / (e.Beta * e.Beta * A);
...
void xMaterial2::Deviations(U_Energy &Energy, xLattice &L)
{
...
doubleU xMaterial2::Lifetime(xRing &ring, xBeam &beam, xLattice &L, doubleU &L2)
{
   ...
   return (sss + sec + snr + sie) * N * L2 / ring.Trev;
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------

xTarget2::xTarget2(xBeam &beam)
{
   ID = 3;
   //OpticName = "XTARGET";
   EffectName = "XTARGET";
...
int xTarget2::OnSet()
{
   ...
   else if (itype == 2)
   {
      ...
      probability = ParticeProbability(t, X, iRing);
   }

   Data.OnSet(36, 6, Ro);
   Data.OnSet(36, 7, N);
   Data.OnSet(36, 9, N * L1);
   Data.OnSet(36, 10, sqrt(Teta2()));
   Data.OnSet(36, 11, dE);
   Data.OnSet(36, 12, sqrt(Estr2()));
   //Data.OnSet(40,6,(Nx * probability)(cm_^-2));
   //Data.OnSet(39,6,(iBeam.N_b * iBeam.Emit[3] * Nx* probability / iRing.Trev)((cm_^-2)*(s_^-1)));

   return 0;
}

vectorU xTarget2::Rates(xTime &time, xBeam &beam, xRing &ring)
{
   probability = U_1;
   vectorU rates(s_ ^ -1);
   vectorU X(m_, U1_);
   ...
   if (itype == 2)
   {
      ...
      probability = ParticeProbability(time, X, ring);
   }
...
bool xTarget2::GetPellet(xTime &t, vectorU &ion)
{
   doubleU x1(m_), y1(m_);
   x1 = ion[0] - PShiftX;
   if (U_Abs(x1) < PelletX / 2)
   {
      y1 = ion[2] + (Velosity * t.t);
      y1 -= Interval * floor((y1 / Interval)(U1_));
      if (U_Abs(y1) < PelletY / 2)
         return true;
      y1 -= Interval;
      if (U_Abs(y1) < PelletY / 2)
         return true;
...
bool xTarget2::GetPelletA(xTime &t, vectorU &ion)
{
   doubleU x1(m_), y1(m_);
   x1 = ion[0] - PShiftX;
   if (U_Abs(x1) < PelletX / 2)
...
//---------------------------------------------------
doubleU xTarget2::Probability(xBeam &beam)
{
...
doubleU xTarget2::ParticeProbability(xTime &t, vectorU &ion, xRing &ring)
{
   doubleU probability(1, U1_);
   doubleU Horda2(m_ ^ 2);
   doubleU Horda(m_);
   ...
   if (Ah() == 0)
   {
      if (((Lattice.Dx * ion[5]) > (PShiftX - XWidth)) && ((Lattice.Dx * ion[5]) < (PShiftX + XWidth)))
...
doubleU xTarget2::Ncross(xTime &t, vectorU &ion, xRing &ring)
{
   doubleU Nc(U1_);
...
//---------------------------------------------------------
void xTarget2::Kick(xTime &time, xBeam &beam, xRing &ring)
{
   ...
   */
   for (int j = 0; j < beam.Number(); j++)
   {
      ...
      {
         if (itype == 1)
            NN = Ncross(time, beam(j), ring);
         if (itype == 2)
            NN = turn * ParticeProbability(time, beam(j), ring);
         Ntot += NN;
         if (!beam.Loss(Lattice, j, (loss * NN() / turn)))
         {
            ...
            if (Real)
            {
               ...
               dE_urban = 0.0;
               //if (vzfp==NULL) vzfp=fopen("myfile.txt","wt");
               for (int kk = 0; kk < int(NN()); kk++)
               {
                  ...
                  for (int ll = 0; ll < xDistributor::Poisson(Nion()); ll++)
                  {
                        ...
                        do
                     {
                        ...
                        dE_urban += I/(1.0 - (GGG()*xx));
                        */
                     /*
                        doubleU dE_urban2 = I/(1.0 - (GGG()*double(rand()) / RAND_MAX));
                        dE_urban+=dE_urban2;
                        fprintf(vzfp,"%25.15g\n",dE_urban2);
                        */
                  }
   ...
   if (itype >= 1)
   {
      ...
      meanE1 +=beam[j][5];
      meanE1 /= beam.Number();
      iColl.Luminosity = beam.N_b * beam.Emit[3] * Nx * (meanE-meanE1)/
                         ((dPB2 ^0.5)*turn*ring.Trev);
      */
      iColl.Luminosity = beam.N_b * beam.Emit[3] * Nx * Ntot /
                         (turn * beam.Number() * ring.Trev);
   }
}
