...
char *BData::OnGetC(int r, int c, char *def)
{
   if ((LGetSize() > r) && (LList[r]->LGetSize() > c))
      return (*this)(r, c);
   if (check)
      Warning("Input parameter [", r, ",", c, "] was initialised with default value");
   return def;
}
...
bool BData::OnGetB(int r, int c, bool def)
{
   ...
   if (check)
      Warning("Input parameter [", r, ",", c, "] was initialised with default value:", (int)def);
   return def;
}
...
int BData::OnGetI(int r, int c, int def)
{
   if ((LGetSize() > r) && (LList[r]->LGetSize() > c) && ((*this)[r][c] != EmptyData))
      return (int)(*this)[r][c];
   if (check)
      Warning("Input parameter [", r, ",", c, "] was initialised with default value:", def);
   return def;
}
