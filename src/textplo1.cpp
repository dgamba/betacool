...
void axis(double &min, double &max, BData &data, int col)
{
   ...
   if (remainder < log10(2))
      maximum = 2;
   else if (remainder < log10(4))
      maximum = 4;
   else if (remainder < log10(8))
      maximum = 8;
   else
      maximum = 10;
...
int main(int argc, char *argv[])
{
   ...
   while (true)
   {
      ...
      {
         if (param.Load(argv[p]) == 0)
            return 1;
         if (graf.Load(param[0](0).ch.get()) == 0)
            return 1;
         ...
         if (p == 1)
         {
            if (param[2][0] != EmptyData)
               timer = param[2][0];
            if (param[3][0] != EmptyData)
               stoptime = param[3][0];
         }
   ...
   */
         ...
         if (param[8][0] != EmptyData)
            Ymax = param[8][0];
         if (param[9][0] != EmptyData)
            Ymin = param[9][0];
         ...
         if (param[10][0] != EmptyData)
            Xmax = param[10][0];
         if (param[11][0] != EmptyData)
            Xmin = param[11][0];
   ...
   */
         int x, y;
         char up, lo, jo;
         if (param[12][0] != EmptyData)
            up = char(param[12][0]);
         if (param[13][0] != EmptyData)
            lo = char(param[13][0]);
         if (param[14][0] != EmptyData)
            jo = char(param[14][0]);
