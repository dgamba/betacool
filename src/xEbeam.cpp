...
if (model == 5)
{
   ...
   if(Minst.Number < 1) Minst.Number = 1;
   if(Minst.Number > Bunch.Number) Minst.Number = Bunch.Number;
...
vectorU xEbeam::Ibeam2Ebeam(xTime&t, vectorU ion)
{
   ...
   switch(model)
   {
    case 0: ion = UniCilinder(t, ion);   break;
    case 1: ion = UniBunch(t, ion);      break;
    case 2: ion = GaussCilinder(t, ion); break;
    case 3: ion = GaussBunch(t, ion);    break;
    case 4: ion = HollowBeam(t, ion);    break;
    case 5: ion = Array(t, ion);         break;
    case 6: ion = Parabolic(t,ion);      break;
    case 7: ion = DensityFile(t,ion);    break;
   }
   return ion;
}
//---------------------------------------------------------------------------
...
vectorU xEbeam::HollowBeam(xTime&t, vectorU&ion)
{
   ...
   if (SpaceCharge)
   {
      ...
      ion_new[5] -= UC_dp_P(rc, I_circle, r_circle)
                  - UC_dp_P(rc, I_hocle,  r_hole)
                  + UC_dp_P(rc, I_hole,   r_hole);

      Vdr       = UC_Vdrift(rc, I_circle, r_circle)
                - UC_Vdrift(rc, I_hocle,  r_hole)
                + UC_Vdrift(rc, I_hole,   r_hole);
...
vectorU xEbeam::Array(xTime&t, vectorU&ion) // models electron beam like a bunch with Gaussian distribution
{
   ...
   for (int i = 0; i < Minst.Number; i++)
         if(
             ...
             (
                ...
                ((Local[Minst[i]][2]-ion_local[2]()) * (Local[Minst[i]][2]-ion_local[2]())/(Mean2[2]*Box*Box))
                )<1.0
             )
           ) Loc_n++;

    F.n_e = Loc_n*ANe /(2.0*M_PI* Box*Box*sqrt(1.0* Mean2[0]* Mean2[2]* Mean2[4])*
   ...
   */
   ...
   n_loc_an = ANe* U_Exp(((-1.0*ion_local[0]*ion_local[0])/(2.0*Asigma[0]*Asigma[0])) +
           ((-1.0*ion_local[2]*ion_local[2])/(2.0*Asigma[2]*Asigma[2])) +
           ((-1.0*ion_local[4]*ion_local[4])/(2.0*Asigma[4]*Asigma[4]*e_Energy.Gamma*e_Energy.Gamma)) )/
           (2.0*M_PI*sqrt(2.0*M_PI)*Asigma[0]*Asigma[2]*Asigma[4]*e_Energy.Gamma);
   if(BiGauss)
   {
      n_loc_an *= (Bunch.Number-N_core)/double(Bunch.Number);
      n_loc_an += (N_core*ANe/Bunch.Number)* U_Exp(((-1.0*ion_local[0]*ion_local[0])/(2.0*sigma_core*sigma_core*Asigma[0]*Asigma[0])) +
           ((-1.0*ion_local[2]*ion_local[2])/(2.0*sigma_core*sigma_core*Asigma[2]*Asigma[2])) +
           ((-1.0*ion_local[4]*ion_local[4])/(2.0*sigma_core*sigma_core*Asigma[4]*Asigma[4]*e_Energy.Gamma*e_Energy.Gamma)) )/
