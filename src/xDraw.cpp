   ...
   if (evolution || cansave)
   {
      ...
      if (beam.benum == BUCKET)
      {
         ...
         if (iRing.Bucket.Analytic && (iRing.Bucket.Stationary || iRing.Bucket.Moving))
         {
            ...
            int a = r / 2;
            //***********************************************************************************************************
            double WWmax = 0, xx1 = 0, xx2 = 0;
            for (int j = -a; j <= a; j++)
            {
               X[0] = iRing.Circ(m_) * j / (iRing.h() * r);
               doubleU V(0, V_), V0(0, V_);
               if (iRing.Bucket.Induction)
               {
               ...
               X[3] = -iRing.V(k_3 * V_) * sin(X[0] * iRing.h() * 2 * M_PI / iRing.Circ()) - iRing.V_low(k_3 * V_) * sin(X[0] * iRing.h_low() * 2 * M_PI / iRing.Circ()) - V(k_3 * V_);
            ...
            for (int j = -a; j <= a; j++)
            {
               ...
               X[4] = -X[1];
               X[6] = -V(k_3 * V_);
               X[2] = -iRing.V(k_3 * V_) * sin(X[0] * iRing.h() * 2 * M_PI / iRing.Circ()) - iRing.V_low(k_3 * V_) * sin(X[0] * iRing.h_low() * 2 * M_PI / iRing.Circ());
               xx1 = iRing.V(k_3 * V_);
               xx2 = V0(k_3 * V_);
               Barrier.Points(X, 7);
            }
...
bool xDraw::DrawBeamEvolution(xBeam &beam, xTime &itime, xColl &coll, int period)
{
   ...
   {
      Warning("Tcal[m.s]=", (double)EvolutionTimer.minutes + (double)(EvolutionTimer.seconds % 60) / 100.,
...
void xDraw::SpaceChargeDraw(xBeam &beam, xEbeam &ebeam)
{
   ...
   for (int i1 = 0; i1 <= steps * 2; i1++)
   {
      ...
      if (ebeam.model == 4)
      {
         ...
         dP = ebeam.UC_dp_P(r, ebeam.I_circle, ebeam.r_circle) - ebeam.UC_dp_P(r, ebeam.I_hocle, ebeam.r_hole) + ebeam.UC_dp_P(r, ebeam.I_hole, ebeam.r_hole);
         SpaceCharge.Point(r(m_), dP());

         Vdr = ebeam.UC_Vdrift(r, ebeam.I_circle, ebeam.r_circle) - ebeam.UC_Vdrift(r, ebeam.I_hocle, ebeam.r_hole) + ebeam.UC_Vdrift(r, ebeam.I_hole, ebeam.r_hole);
...
double xDraw::DrawPellet(double L)
{
   ...
   BeamSize.Size(5, 3);
   X[0] = iPellet.pellets[iPellet.Nplts - 1].position[4];
   X[1] = -a[0] * 1e3;
   X[2] = -a[1] * 1e3;
   BeamSize.Points(X, 3);
   X[0] = 0;
...
void xDraw::TuneShift()
{
   ...
   if (j < 0)
      j = 0;
   if (j >= iIBS.iTune.xyz[1])
      j = iIBS.iTune.xyz[1] - 1;
