      ...
      do
      {
         ...
         for (int i = 0; i < 3; i++)
         {
            if (iBeam.Emit[i]() <= 0) // Check on negative values
               negative = true;
            else if (((iBeam.Emit[i] / emit0[i])() > ratio) || // Check on too large growth
         ...
         if (Synchrotron == 2)
         {
            ...
            if (LongSC)
            {
               ...
               for (int j = 0; j <= iDraw.Divisions; j++)
               {
                  ...
                  if (j)
                     LongHyst[2][j] = g * (Xo - LongHyst[1][j]);
                  else
                     LongHyst[2][j] = 0;
                  Xo = LongHyst[1][j];
...
#pragma omp parallel for
      ...
      else if (iBeam.benum == BUCKET)
      {
         ...
         else if (iRing.Bucket.Moving || iRing.Bucket.Stationary) // 2 sin harmonics (Ring | Barrier Bucket | Harmonic RF)
         {
            if (iRing.Bucket.Moving)
            {
               iRing.Bucket.SetMoving(dtime.t);
...
#pragma omp parallel for
                  ...
                  else
                  {
                     if (fabs(iBeam[j5][4]) < RFlength(m_))
                     {
                        if (iBeam[j5][4] > 0)
                        {
                           ...
                           if (iBeam[j5][5] < 0)
                              T_curr = (2.0 * iBeam.t_1) + (2.0 * iBeam.t_2) - T_curr;
                        }
                        else
                        {
                           T_curr = iBeam.T_s + ((iBeam[j5][4] / RFlength(m_)) * iBeam.t_1);
                           if (iBeam[j5][5] < 0)
                              T_curr = (2.0 * iBeam.t_1) + (2.0 * iBeam.t_2) + (iBeam.T_s - T_curr);
                        }
                     }
                     else
                     {
                        if (iBeam[j5][4] > 0)
                        {
                           if (iBeam[j5][5] > 0)
                              ...
                              T_curr = iBeam.t_1 + (2.0 * iBeam.t_2) - (((iBeam.Amp2 ^ 0.5) + iBeam[j5][5]) * iRing.Circ * iRing.Energy.Momentum / (U_e * iRing.Energy.Z * iRing.V_B));
                        }
                        else
                        {
                           if (iBeam[j5][5] > 0)
                              T_curr = (3.0 * iBeam.t_1) + (4.0 * iBeam.t_2) - (((iBeam.Amp2 ^ 0.5) - iBeam[j5][5]) * iRing.Circ * iRing.Energy.Momentum / (U_e * iRing.Energy.Z * iRing.V_B));
                           else
                              T_curr = (3.0 * iBeam.t_1) + (2.0 * iBeam.t_2) + (((iBeam.Amp2 ^ 0.5) + iBeam[j5][5]) * iRing.Circ * iRing.Energy.Momentum / (U_e * iRing.Energy.Z * iRing.V_B));
                        }
                     }
                     T_int = T_curr + dtime.dt;
                     while (T_int > iBeam.T_s)
                        T_int -= iBeam.T_s;
                  }
      ...
      for (int j1 = 0; j1 < iBeam.Number(); j1++)
      {
         ...
         if (iRing.Inductor)
            iBeam.Add(j1, 5, U_e * iRing.Vind * iTime.dt / iRing.Trev / (iRing.Energy.Momentum * iRing.Energy.Velocity * iRing.Energy.Gamma));
      }
      ++iTime;
      ++jtime;
      ++etime;
...
void xDynamics::Tracking()
{
   ...
   while (!iBeam.inject || (cycles < iBeam.cycles))
   {
      if (iBeam.inject)
      {
         ...
         int num = 0;
         if (cycles)
         { // no new injection for 1st cycle
            if (iLosses.Multi && iLosses.Injection)
            {
               //bool linj_loss = true;
               int Nmp = iBeam.Number();
         ...
         for (int steps = 0; steps < Steps; steps++)
         {
            ...
            else
            {
               if (opt.LGetSize())
               {
                  ...
                  else if ((iLaserCooling.Multi != 0) && (opt[0] == &iLaserCooling))
                  {
                     for (int j = 0; j < iBeam.Number(); j++)
...
#pragma omp parallel for
      ...
      if (iRing.Inductor)
         for (int j1 = 0; j1 < iBeam.Number(); j1++)
            iBeam.Add(j1, 5, U_e * iRing.Vind / (iRing.Energy.Momentum * iRing.Energy.Velocity * iRing.Energy.Gamma));
      if (traj)
