...
void xRestGas::Kick(xTime &time, xBeam &beam, xRing &ring)
{
   ...
   for (int j = 0; j < beam.Number(); j++)
   {
      ...
      {
         beam.Add(j, 1, (U_Abs((beam.Emit[0] / Lattice.betax) * rates[0] * time.dt * 2.0) ^ 0.5) * xDistributor::Gaussian());
         beam.Add(j, 3, (U_Abs((beam.Emit[1] / Lattice.betay) * rates[1] * time.dt * 2.0) ^ 0.5) * xDistributor::Gaussian());
         beam.Add(j, 5, (U_Abs(beam.Emit[2] * rates[2] * time.dt * 2.0) ^ 0.5) * xDistributor::Gaussian());
      }
   }
}
