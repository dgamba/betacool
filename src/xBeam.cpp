...
int xBeam1::OnSet()
{
   ...
   if (iMomentum < 4)
      ...
      Data.OnSet(1, 21, ((Emit[2] ^ 0.5) * pEnergy->Kinetic * (pEnergy->Gamma + 1) / pEnergy->Gamma)(uMomentum));
...
{ /*
   ...
   */
   return (U_rp * 2 * (Emit[3] ^ 3) * (pEnergy->Z ^ 2) * ((*pBetaH * emit) ^ 0.5) /
...
int xBeam::CalcStability()
{
   ...
   if (a < (*pVac_a))
   {
      ...
      Warning("Mean beam radius= ", a(), "  is more than chamber ", (*pVac_a)());
...
void xBeam::CalcBunch()
{
   ...
   if (Emit[3] < N_max)
      Q_s = (*pRF_Q_s) * (((-Emit[3] / N_max) + 1) ^ 0.5);
   else
      Q_s = 0.0;
...
void xBeam::GetBucket(vectorU &X, doubleU &T_int)
{
   ...
   else
   {
      ...
      {
         X[4] = (U_Abs(pRing->Eta) * pRing->Energy.Velocity * (Amp2 ^ 0.5) * T_int) - (U_Abs(pRing->Eta) * pRing->Energy.Velocity * U_e * pRing->Energy.Z * pRing->V_B * (T_int - t_1) * (T_int - t_1) /
         ...
         else
         {
            if (T_int < ((3.0 * t_1) + (4.0 * t_2)))
            {
               X[4] = (-0.5 * pRing->Energy.Velocity * pRing->T2) -
                      (U_Abs(pRing->Eta) * pRing->Energy.Velocity * (Amp2 ^ 0.5) * (T_int - (3.0 * t_1) - (2.0 * t_2))) + (U_Abs(pRing->Eta) * pRing->Energy.Velocity * U_e * pRing->Energy.Z * pRing->V_B * (T_int - (3.0 * t_1) - (2.0 * t_2)) * (T_int - (3.0 * t_1) - (2.0 * t_2)) / (2.0 * pRing->Circ * pRing->Energy.Momentum));
...
int CollidingBeam::OnGet()
{
   ...
   case 2:
      ...
      if (Energy.Set(U_KINETIC, Kinetic, UKinetic))
         return 2;
      break;
   case 3:
