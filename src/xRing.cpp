...
int xRing::OnGet()
{
   ...
   case 0:
      ...
      if (Energy.Set(U_GAMMA))
         return 2;
      break;
   case 1:
      Energy.Beta = Data.OnGet(11, 3, 0.99);
      if (Energy.Set(U_BETA))
         return 2;
      break;
   case 2:
      if (Energy.Set(U_KINETIC, Data.OnGet(11, 4, 1), UKinetic))
         return 2;
      break;
   case 3:
      Energy.Momentum = Data.OnGet(11, 5, 1);
      if (Energy.Set(U_MOMENTUM))
         return 2;
      break;
   }
...
int xRing::OnRun()
{
   ...
   if (iBeam.benum == BUCKET)
   {
      ...
      else
      {
         ...
         for (int i = 1; i < 5; i++)
            Bucket.barrier[i].UP = (Bucket.barrier[i].s1 - Bucket.barrier[i].s2) * Bucket.barrier[i].Ub + Bucket.barrier[i - 1].UP;
...
int xRing::OnSet()
{
   ...
   if (once)
   {
      ...
      if (LatticeFile < 2)
      {
         if (LatticeFile == 0)
         {
            if (OutputMAD(Data.OnGetC(13, 4, "NoFile")))
               return 1;
         }
         else
         {
            if (InputMAD(Data.OnGetC(13, 6, "NoFile")))
               return 1;
         }
         switch (ReduceExtend)
         {
   ...
   if (LatticeFile == 1)
   {
      ...
      {
      case 0:
         Matrix = GetMatrix(iTime, Index20);
         break;
      case 1:
         Matrix = RingMatrix(iTime, Index20);
         break;
      }
      double m2d;
      for (int i = 0; i < 6; i++)
      ...
      if (ring[i].Length > extend)
      {
         ...
         for (int j = 0; j < Steps - 1; j++)
         {
            ...
            for (int k = 0; k < ring[i].LGetSize(); k++)
            {
               ...
               else if (ring[i][k]->OpticName == "EBEND")
               {
               ...
               else if (ring[i][k]->OpticName == "FFIELD")
               {
               ...
               else if (ring[i][k]->OpticName == "QUADRUPOLE")
               {
      ...
      if (plib)
      {
         ...
         if (plib->OpticName == "SBEND")
         {
            ...
            for (int j = 2; j < MadLine.Col(i); j++)
            {
               ...
               else if (MadLine[i](j).ch == "E1")
                  bend->E1 = MadLine[i][j + 1];
               else if (MadLine[i](j).ch == "E2")
                  bend->E2 = MadLine[i][j + 1];
               else if (MadLine[i](j).ch == "K1")
                  bend->K1 = MadLine[i][j + 1];
               else if (MadLine[i](j).ch == "HGAP")
               {
                  bend->HGAP1 = MadLine[i][j + 1];
                  bend->HGAP2 = MadLine[i][j + 1];
               }
               else if (MadLine[i](j).ch == "FINT")
               {
             ...
             if (plib->OpticName == "EBEND")
         {
            ...
            for (int j = 2; j < MadLine.Col(i); j++)
            {
               ...
               else if (MadLine[i](j).ch == "FB")
                  ebend->FB = MadLine[i][j + 1];
               else if (MadLine[i](j).ch == "FE")
                  ebend->FE = MadLine[i][j + 1];
               else if (MadLine[i](j).ch == "N")
         ...
         else if (plib->OpticName == "SOLENOID")
         {
         ...
         else if (plib->OpticName == "RFCAVITY")
         {
...
bool xRing::OutputMAD(char *FileName)
{
   ...
   */
   ...
   {
      if (!MADFile.Load(FileName, &ColFile))
         return true;
