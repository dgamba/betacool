...
vectorU xEcool::Rates(xTime &t, xBeam &beam, xRing &ring)
{
   ...
   for (int jj = 0; jj < iEbeam.BunchNumber; jj++)
   {
      ...
      {
      case 0:
         rates += SingleParticle(t, beam, ring);
         break;
      case 1:
         rates += MonteCarlo(t, beam, ring);
         break;
      }
...
doubleU xEcool::LifeTime(xTime &t, xBeam &beam, xRing &ring)
{
   ...
   result = (ring.Energy.Z ^ 2) * ((eV / iEbeam.F.Ttemp) ^ 0.5) * (U_Ln(11.32 * ring.Energy.Z / ((iEbeam.F.Ttemp / eV) ^ 0.5)) + (0.14 * ((iEbeam.F.Ttemp / (eV * (ring.Energy.Z ^ 2))) ^ (1. / 3.)))) * alfa;
   lifetime *= (ring.Energy.Z ^ 2) * ((eV / iEbeam.F.Ttemp) ^ 0.5) * (U_Ln(11.32 * ring.Energy.Z / ((iEbeam.F.Ttemp / eV) ^ 0.5)) + (0.14 * ((iEbeam.F.Ttemp / (eV * (ring.Energy.Z ^ 2))) ^ (1. / 3.))));
   ...
   for (int j = 0; j < beam.Number(); j++)
      if (!iEbeam.LongPulse || ((iBeam(j, 4) > iRing.Circ * iEbeam.PulseFrom) && (iBeam(j, 4) < iRing.Circ * iEbeam.PulseUpTo)))
      {
         ...
         for (int jj = 0; jj < iEbeam.BunchNumber; jj++)
         {
            ...
            for (int k = 1; k < 6; k += 2)
            {
               beam(j, k, beam(j, k).Saturation(((X[k] - beam(j, k)) * time.dt * SectionNumber / ring.Trev), iEbeam.Shift[k]));
            }
