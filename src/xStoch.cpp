...
vectorU xStochastic::Rates(xTime &t, xBeam &beam, xRing &ring)
{
   ...
   if (H_use)
   {
      ...
      Hx = U_e * U_e * HG * 3.0 * HA2 * beam.Emit[3] * xx * U_Pow((HK_N * HK_beta * HPU_N * HPU_beta / (xx * xx)), 0.5) *
           HPU_S * HK_S * Z * (1.0 + ring.Energy.Beta) * H_l / (ring.Trev * ring.Trev * 16.0 * U_Abs(ring.Eta) * Msigma * U_Pow(beam.Emit[2], 0.5) * HK_h * HPU_h * ring.Energy.Beta * ring.Energy.Momentum * U_c * HBand);
   ...
   if (V_use)
   {
      ...
      Vx = U_e * U_e * VG * 3.0 * VA2 * beam.Emit[3] * xx * U_Pow((VK_N * VK_beta * VPU_N * VPU_beta / (xx * xx)), 0.5) *
           VPU_S * VK_S * Z * (1.0 + ring.Energy.Beta) * V_l / (ring.Trev * ring.Trev * 16.0 * U_Abs(ring.Eta) * Msigma * U_Pow(beam.Emit[2], 0.5) * VK_h * VPU_h * ring.Energy.Beta * ring.Energy.Momentum * U_c * VBand);
      ...
      if (L_use)
      {
         ...
         else
         {
            ...
            r = ((B * 0.5 * (beam.Emit[2] ^ 0.5) * beam.Emit[3] * ring.Energy.Gamma /
                  (ring.Energy.Kinetic * (1.0 + ring.Energy.Gamma)) * time.dt * k) ^
                 0.5)(U1_);
            beam.Add(j, 5, r * xDistributor::Gaussian());
   ...
   for (i = n_min; i <= n_max; i++)
   {
      if (n_series == 2)
         forces[5] += (2.0 * U_Sin(domega * T_0 * (U_1 + T_n) * h_sub * i) *
                       (U_Cos(domega * T_0 * h_sub * i) - U_1)) *
                      (C_tilda * Rgain) * 2 * U_pi / (T_0 * e.Velocity);
      if (n_series == 1)
         forces[5] += (U_Sin(domega * T_0 * T_n * h_sub * i) -
                       U_Sin(domega * T_0 * (U_1 + T_n) * h_sub * i)) *
                      (C_tilda * Rgain) * 2 * U_pi / (T_0 * e.Velocity);
   }
                  ...
                  if (beam.benum == BUCKET)
                  {
                     ...
                     if (ring.Bucket.barrier[p].P)
                        r = System[i].Rates(beam, ring, ring.Bucket.barrier[p].P,
                                            ring.Bucket.barrier[p].D * beam.Emit[3](), simple, optimal) *
                            time.dt;
                     else
                        r = 0;
                  }
...
double xGated::getDifusion(double dp, double sigma)
{
   if (-sigma <= dp && dp <= 0)
      return -dp;
   else if (0 <= dp && dp <= sigma)
      return dp;
   else if (-2. * sigma <= dp && dp <= -sigma)
      return 2 * sigma + dp;
   else if (sigma <= dp && dp <= 2. * sigma)
      return -(dp - 2 * sigma);
   else
      return 0;
}
