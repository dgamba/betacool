...
double betaRel = sqrt(((pow(gammaRel, 2)) - 1) / pow(gammaRel, 2));
...
double AIBS = (c0 * nb * pow(r0, 2)) / (8. * M_PI * pow(betaRel, 3) * pow(gammaRel, 4) * emittancex * emittancey * sigmab * momentum);
int bem = 1;
double maxy1 = 200;
double maxy2 = 1000;
...
void bpIBSinit(bpBeam &beamParam, bpEmittance &emit)
{
	...
	betaRel = sqrt(((pow(gammaRel, 2)) - 1) / pow(gammaRel, 2));											 //longditudial speed
	...
	AIBS = (c0 * nb * pow(r0, 2)) / (8. * M_PI * pow(betaRel, 3) * pow(gammaRel, 4) * emittancex * emittancey * sigmab * momentum);
...
double coulombLog(bpLattice &Latt)
{
	...
	if (bem == 1)
	{
		v = 8 * sqrt(pow(M_PI, 3) * (Latt.betax * emittancex + pow(Latt.Dx * momentum, 2)) * (Latt.betay * emittancey + pow(Latt.Dy * momentum, 2))) * sigmab;
	}
	else
	{
		v = 4 * M_PI * sqrt((Latt.betax * emittancex + pow(Latt.Dx * momentum, 2)) * (Latt.betay * emittancey + pow(Latt.Dy * momentum, 2))) * sigmab;
	}
...
#pragma omp parallel for reduction(+                                       \
	...
	for (int i = 0; i < ring.size; i++)
	{
		...
		if ((abs(Latt.Dx) < 1e-50) && (abs(Latt.Dpx) < 1e-50))
		{
			Latt.Dx = pow(10, -50);
			Latt.Dpx = pow(10, -50);
		}
		if ((abs(Latt.Dy) < 1e-50) && (abs(Latt.Dpy) < 1e-50))
		{
			Latt.Dy = pow(10, -50);
			Latt.Dpy = pow(10, -50);
		}
...
void bpIbsKick(double dt, bpRates &growth, bpBeam &beam, bpEmittance &emit, bpLattice &latt)
{
	...
	for (int i = 0; i < beam.row; i++)
	{
		if (growth.ratex < 1e-111)
			...
			beam[1][i] += sqrt(emit.emittancex / latt.betax * growth.ratex * dt * 2.) * Gaussian();

		if (growth.ratey < 1e-111)
			beam[3][i] *= exp(growth.ratey * dt);
		else
			beam[3][i] += sqrt(emit.emittancey / latt.betay * growth.ratey * dt * 2.) * Gaussian();

		if (growth.ratez < 1e-111)
			beam[5][i] *= exp(growth.ratez * dt);
		else
			beam[5][i] += sqrt(emit.momentum2 * growth.ratez * dt * kb) * Gaussian();
	}
}
