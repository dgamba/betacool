...
double xIBS::explogint(double D)
{
   ...
   else
   {
      ...
      double x = (((((a6 * D + a5) * D + a4) * D + a3) * D + a2) * D + a1) * D + a0 + am1 / D;
      double D2 = D * D;
...
vectorU xIBS::MartiniIntegral(double a, double b, double c, double d, double d_)
{
   ...
   for (int i = 0; i < stepmu; i++)
   {
      ...
      for (int j = 0; j < stepnu; j++)
      {
         ...
         DCAP = ((sin_mu_2 * sin_nu_2) + (sin_mu_2 * ((a * cos_nu) - (d_ * sin_nu)) * ((a * cos_nu) - (d_ * sin_nu))) + (b * b * cos_mu_2)) / (c * c);
...
vectorU xIBS::Burov(xLattice &lat, vectorU X, xRing &ring)
{
   ...
   K4 = ((lat.Dx * lat.Dx) + ((lat.Dpx * lat.betax) + (lat.alfax * lat.Dx)) *
                                 ((lat.Dpx * lat.betax) + (lat.alfax * lat.Dx))) *
        Vzm * Vzm / x2m;
   /*
...
vectorU xIBS::Gas_Relaxation(xBeam &beam, xRing &ring)
{
  ...
  */
   ...
   if (Model == 4)
      // Use G-R:
      rates[2] = ri * ri * beam.Emit[3] * U_c * 20. / ((8. * ((U_pi / 2.0) ^ 0.5)) * ring.Energy.Gamma3 * ring.Energy.Beta3 * (beam.Emit[0] ^ 1.5) * (Lattice.betax ^ 0.5) * s_s * beam.Emit[2]);
   //  else
   // Use B-M:
   //  rates[2] = ri*ri* beam.Emit[3]* U_c*20./(8.*ring.Energy.Gamma3*ring.Energy.Beta3*
   //             (beam.Emit[0]^1.5)*(Lattice.betax^0.5)*s_s*beam.Emit[2]);
...
vectorU xIBS::Bjorken_Mtingwa_HE(xBeam &beam, xRing &ring)
{
   ...
   rates[2] = ri * ri * beam.Emit[3] * U_c * 20. / (8. * ring.Energy.Gamma3 * ring.Energy.Beta3 * (beam.Emit[0] ^ 1.5) * (Lattice.betax ^ 0.5) * s_s * beam.Emit[2]);
...
vectorU xIBS::Bjorken_Mtingwa2(xLattice &lat, xBeam &beam, xRing &ring)
{
   ...
   for (int k = 0; k < step_lam; k++)
   {
      ...
      //LB = LL.Back();
      LB[0][0] = (LL[1][1] * LL[2][2]) - (LL[1][2] * LL[2][1]);
      LB[0][1] = (LL[2][0] * LL[1][2]) - (LL[1][0] * LL[2][2]);
      LB[0][2] = 0;
      LB[1][0] = LB[0][1];
      LB[1][1] = (LL[0][0] * LL[2][2]) - (LL[0][2] * LL[2][0]);
      LB[1][2] = (LL[2][0] * LL[0][1]) - (LL[0][0] * LL[2][1]);
      LB[2][0] = 0;
      LB[2][1] = LB[1][2];
      LB[2][2] = (LL[0][0] * LL[1][1]) - (LL[0][1] * LL[1][0]);
      for (int i = 0; i < 3; i++)
         for (int j = 0; j < 3; j++)
            LB[i][j] /= det;
...
void xIBS::Kick(xTime &time, xBeam &beam, xRing &ring)
{
   ...
   case 0:
      ...
      else if (Slices && beam.benum != COASTING)
      {
         ...
         for (i = 0; i < 3; i++)
            if ((i != 2) || (beam.benum == BUNCHED))
            {
               ...
               if (fabs(beam.Fitting[i][1]) > fabs(beam.Fitting[i][2]))
               {
                  ...
                  Nc[i] = 1. / (1. + fabs(beam.Fitting[i][1] / beam.Fitting[i][2] *
               ...
               else
               {
                  ...
                  Nc[i] = 1. / (1. + fabs(beam.Fitting[i][2] / beam.Fitting[i][1] *
                          ...
                          vectorU rates, vectorU &emit, double k)
{
   ...
   if (rates[0]() < 0)
      ...
      beam.Add(j, 1, (((emit[0] / Lattice.betax) * rates[0] * time.dt * k * 2.0) ^ 0.5) * xDistributor::Gaussian());

   if (rates[1]() < 0)
      beam(j, 3, beam(j, 3) * U_Exp(rates[1] * time.dt * k));
   else
      beam.Add(j, 3, (((emit[1] / Lattice.betay) * rates[1] * time.dt * k * 2.0) ^ 0.5) * xDistributor::Gaussian());

   if (rates[2]() < 0)
      beam(j, 5, beam(j, 5) * U_Exp(rates[2] * time.dt * k));
   else
      beam.Add(j, 5, ((emit[2] * rates[2] * time.dt * k * kb) ^ 0.5) * xDistributor::Gaussian());
}
...
void xIBS::LocalKick(xTime &time, xBeam &beam, xRing &ring)
{
   ...
   if (beam.Minst.Number < 1)
      beam.Minst.Number = 1;
   if (beam.Minst.Number > beam.Bunch.Number)
      beam.Minst.Number = beam.Bunch.Number;
   ...
   for (int n = 0; n < ring.Number(); n++)
   {
      ...
      for (int j1 = 0; j1 < beam.Number(); j1++)
      {
         ...
         */
         ...
         n_loc = Loc_n * beam.Emit[3] / (2.0 * M_PI * Box_IBS * Box_IBS * sqrt(1.0 * beam.Mean2[0] * beam.Mean2[2] * beam.Mean2[4]) * beam.Bunch.Number * lengthU * lengthU * lengthU);
         ...
         if (n_loc() > 0.0)
         {
            ...
            for (int i = 0; i < beam.Minst.Number; i++)
            {
               ...
               if (U_rel() != 0.0)
               {
                  ...
                  {
                     if (theta() > 0.)
                        delta_v[a] += (v[a] - v_local[a]) * B_M_log / (U_rel ^ 1.5);

                     for (int b = 0; b < 3; b++)
                        if (a == b)
                           delta_d[a][b] += ((U_rel - ((v[a] - v_local[a]) * (v[b] - v_local[b]))) * B_M_log) / (U_rel ^ 1.5);
                        else
                           delta_d[a][b] += -((v[a] - v_local[a]) * (v[b] - v_local[b])) * B_M_log / (U_rel ^ 1.5);
                  }
               }
            }
            for (int j = 0; j < 3; j++)
            {
               if (theta() > 0.)
                  f[j] = -delta_v[j] * delta / beam.Minst.Number;
            ...
            */
            ...
            ion_local[1] += ((Cz4 * time.dt * ring[n].Length / ring.Arc / ring.Energy.Gamma) ^ 0.5) * ksi1 / (ring.Energy.A * U_amu);
            ion_local[3] += ((0.5 * D[1][1] * time.dt * ring[n].Length / ring.Arc / ring.Energy.Gamma) ^ 0.5) * ksi1 / (ring.Energy.A * U_amu);
            ion_local[3] += ((0.5 * D[1][1] * time.dt * ring[n].Length / ring.Arc / ring.Energy.Gamma) ^ 0.5) * ksi2 / (ring.Energy.A * U_amu);
            ion_local[5] += ((Cz1 * time.dt * ring[n].Length / ring.Arc / ring.Energy.Gamma) ^ 0.5) * ksi1 / (ring.Energy.A * U_amu);
            ion_local[5] += ((Cz2 * time.dt * ring[n].Length / ring.Arc / ring.Energy.Gamma) ^ 0.5) * ksi2 / (ring.Energy.A * U_amu);
            ion_local[5] += ((Cz3 * time.dt * ring[n].Length / ring.Arc / ring.Energy.Gamma) ^ 0.5) * ksi3 / (ring.Energy.A * U_amu);
...
*/
   ...
   Ac = U_c * ri * ri * Numc * B_M_log / (8. * U_pi * Ec[0] * Ec[1] * s_s * (Ec[2] ^ 0.5) * ring.Energy.Beta3 * ring.Energy.Gamma2 * ring.Energy.Gamma2);

   At = U_c * ri * ri * Numt * B_M_log / (8. * U_pi * Et[0] * Et[1] * s_s * (Et[2] ^ 0.5) * ring.Energy.Beta3 * ring.Energy.Gamma2 * ring.Energy.Gamma2);

   Act = U_c * ri * ri * Numct * B_M_log / (8. * U_pi * Ect[0] * Ect[1] * s_s * (Ect[2] ^ 0.5) * ring.Energy.Beta3 * ring.Energy.Gamma2 * ring.Energy.Gamma2);
   ...
   if (uplimit < real(Lct[1][1]))
      uplimit = real(Lct[1][1]);
   if (uplimit < real(Lct[2][2]))
      uplimit = real(Lct[2][2]);
   uplimit *= lam_limit;
