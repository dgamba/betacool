...
vectorU xHeat::Rates(xTime&t, xBeam& beam, xRing& ring)
{
  	...
  	for (int i = 0; i < 3; i++)
   {
      ...
      if (diffusion)
      {
         ...
         else rates[i] += Diffusion[i] * num / (beam.Emit[i]^2) * 2;
      }
   }
   return rates;
}
...
void xHeat::Kick (xTime&time, xBeam&beam, xRing&ring)
{
   ...
   {  
      ...
      {  for (int i = 0; i < 2; i++)
         ...
         beam(j, 5, beam(j, 5) * r[2] +
         (tapered * beam(j, 0) * ring.Energy.Gamma2 / ring.DispH));
      }
   }
   if (linear)
   {  
   ...
   if (diffusion)
   {
      ...
      if (difP) r[2] = ((k*DiffusP*time.dt )^0.5)(U1_);
      else      r[2] = ((k*Diffusion[2]*time.dt*2/beam.Emit[2])^0.5)(U1_);
   ...
   if (CollBeam)
      Luminosity = beam.N_b * beam.Emit[3] * cBeam.Emit[3] /
         ((((cBeam.Sigma_x*cBeam.Sigma_x + beam.Sigma_2[0]) *
            (cBeam.Sigma_y*cBeam.Sigma_y + beam.Sigma_2[2]))^0.5) *
            (ring.Trev) * U_pi* 2.0);
   else
...
//11.12.2007
   if(Include)
   {
      ...
      r1 = (((0.25 * Diffusion * Diffusion * Kappa_x * Kappa_x *U_pi*U_pi)*time.dt*2 *
            beam.Emit[0]/( Lattice.betax * ring.Trev))^0.5)(U1_);
      r2 = (((0.25*Diffusion * Diffusion * Kappa_y * Kappa_y *U_pi*U_pi)*time.dt*2 *
             beam.Emit[1]/(Lattice.betay*ring.Trev))^0.5)(U1_);
      if (CollBeam)
      {
